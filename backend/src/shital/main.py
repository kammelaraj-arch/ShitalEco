...
async def _seed_api_key_metadata() -> None:
    ...
    KNOWN_KEYS = [  # noqa: N806
        ...
        ("MAIL_AGENT_USER",           "Mail-agent service-account email for delegated (ROPC) Graph auth — leave blank to use app-only auth via MS_CLIENT_SECRET", "Microsoft", False),
        ("MAIL_AGENT_PASSWORD",       "Mail-agent service-account password (ROPC). Account must NOT have MFA. Leave blank for app-only auth", "Microsoft", True),
        ("TEAMS_WEBHOOK_URL",         "Microsoft Teams incoming-webhook URL for mail-agent alerts (large invoices, complaints, escalations)", "Microsoft", True),
