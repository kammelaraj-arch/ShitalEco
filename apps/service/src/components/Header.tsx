...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur-md"
      style={{ background: 'rgba(140,0,0,0.94)', borderBottom: '1px solid rgba(212,175,55,0.2)' }}>
        ...
        <button onClick={() => setScreen('browse')} className="flex items-center gap-3 min-w-0">
          ...
          <div className="hidden sm:block min-w-0">
            ...
            <div className="flex items-center gap-1.5">
              <p className="text-xs text-ivory-mute font-medium truncate">
            ...
            <AnimatePresence>
              {showLang && (
                <motion.div
                  ...
                  style={{ background: '#1A0606', border: '1px solid rgba(212,175,55,0.25)' }}>
                  {(Object.keys(LANGUAGE_META) as Language[]).map((lang) => (
                    <button key={lang} onClick={() => { setLanguage(lang); setShowLang(false) }}
                      ...
                      style={{
                        ...
                        color: lang === language ? '#D4AF37' : 'rgba(255,248,220,0.65)',
                      }}>
