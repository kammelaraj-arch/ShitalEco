...
# Container/DB settings — read from .env if present, fall back to prod defaults
DB_CONTAINER="${DB_CONTAINER:-shitaleco-db-1}"
DB_NAME="${POSTGRES_DB:-shitaleco_db}"
DB_USER="${POSTGRES_USER:-shitaleco_db_user}"

# Load env file for secrets (Azure creds, DB password)
if [ -f "$ENV_FILE" ]; then
  set -a
  # shellcheck disable=SC1090
  source <(grep -E '^(POSTGRES_PASSWORD|POSTGRES_DB|POSTGRES_USER|DB_CONTAINER|AZURE_STORAGE_CONNECTION_STRING|AZURE_STORAGE_CONTAINER|AZURE_STORAGE_ACCOUNT|AZURE_STORAGE_KEY)=' "$ENV_FILE" 2>/dev/null || true)
  set +a
  # Re-apply after sourcing (env file may override)
  DB_CONTAINER="${DB_CONTAINER:-shitaleco-db-1}"
  DB_NAME="${POSTGRES_DB:-shitaleco_db}"
  DB_USER="${POSTGRES_USER:-shitaleco_db_user}"
fi
...
if docker exec -e PGPASSWORD="$PGPASSWORD" "$DB_CONTAINER" pg_dump -U "$DB_USER" -d "$DB_NAME" --clean --if-exists 2>>"$LOG_FILE" | gzip > "$DAILY_FILE"; then
